...
#define CANON_TOOL_MAX 256      // max size of carousel handled
...
extern void USE_LENGTH_UNITS(int u);

//  Call a comp off command., this will stop processing till comp is done, thus holding the state.

extern void COMPOFF( );

//call a CompOn command
extern void COMPON( double radius, int index, int side );
...
// update the line number for canned motion dwells.

extern void HOLDLINE();

extern void SETCYCLEOFF(); //for signaling end of a canned cycle;

// hold this line number for display tracking...

extern void SETSAFE(double safe);

//for setting Safe_Z
extern void SETCSS();
extern void SETRPM();

extern void CALLSUBROUTINE( int subnumber, int repeat );

extern void CALLSUBROUTINE( LPCTSTR filename , int repeat);

/* Used to call a subroutine by number, repeat n times.. */

extern void RETURNFROMSUBROUTINE();

// return from one nest of subroutine. Users problem to sync these things.

extern void REFAXIS( int axis);

// reference an axis,..send it to home switches..

extern void RADCIRCLECW( double radius );
    
extern void RADCIRCLECCW( double radius );

// these two do a radius cut from a circle center

extern void CALLMACRO( int macro, double,double,double);

// call an M Macro from 20 -97 except 30
...
extern void STRAIGHT_FEED(
 double x, double y, double z, double a_position, double b_position, double c_position);

extern void STRAIGHT_THREAD(
 double x, double y, double z, double a_position, double b_position, double c_position);
...
extern void STRAIGHT_PROBE ( double x, double y, double z, double a, double b, double c );
...
/* Tool Functions */
extern void USE_TOOL_LENGTH_OFFSET(double lengthz, double lengthx);
