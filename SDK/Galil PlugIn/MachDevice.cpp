...
extern void myNotify(int message);  							// you can influence subsequent init by actions here
...
extern void myPurge( short flags );                               
...
extern "C" __declspec(dllexport) void Update() ////////////////////  UPDATE LOOP 10 Times a Second.
{
    ...
    AFX_MANAGE_STATE(AfxGetStaticModuleState( ));
    KickTimer = true;
	if( !TimerOn ) SetTimer(NULL, 1, 100, MYProc);
	myUpdate ();
}   // Update
